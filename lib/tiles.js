...
export function analyzeHand(concealed, exposedCount = 0) {
  ...
  // Count only sets she genuinely HOLDS, so the panel never over-claims:
  //  - three of a kind, or
  //  - a real pair finished with a single Joker.
  // We deliberately do NOT pad a lone tile with two Jokers or count all-Joker
  // sets here — that made a fresh, joker-heavy hand of singles look like it had
  // sets "already locked in", which is confusing. Spare Jokers stay a resource.
  let concealedSets = 0;
  for (const k of Object.keys(counts)) {
    while (counts[k] >= 3) { counts[k] -= 3; concealedSets++; }
  }
  for (const k of Object.keys(counts)) {
    if (counts[k] === 2 && jk >= 1) { counts[k] -= 2; jk -= 1; concealedSets++; }
  }
...
/**
 * Given a winning 14-tile hand, return ONE concrete way it forms 4 sets + a
 * pair, as actual tile objects: { sets: [[t,t,t] x4], pair: [t,t] }, or null
 * if it isn't a win. Used to SHOW her how she won (the win-checker re-arranges
 * all 14 tiles, so the winning grouping isn't always the one she locked).
 * Mirrors isWinningHand: the pair is two identical REAL tiles; jokers are wild
 * inside sets only.
 */
export function decomposeWin(tiles) {
  ...
  // buildSets: carve `needed` triplets out of the remaining real tiles + jokers.
  const buildSets = (rem, jk, needed) => {
    ...
    // Spend three jokers as their own set.
    if (jk.length >= 3) {
      const res = buildSets(rem, jk.slice(3), needed - 1);
      if (res) return [jk.slice(0, 3), ...res];
    }
    if (keys.length === 0) return null;
    const k = keys[0], have = rem[k].length, tries = [];
    if (have >= 3) tries.push([3, 0]);
    if (have >= 2 && jk.length >= 1) tries.push([2, 1]);
    if (have >= 1 && jk.length >= 2) tries.push([1, 2]);
    for (const [r, j] of tries) {
