...
test("analyzeHand: only LOCKED (exposed) sets show as done; in-hand groups are in progress", () => {
  // 1 exposed (locked) set; concealed has a triple + pairs she hasn't committed.
  const concealed = [...reps("bam1", 3), ...reps("crak5", 2), ...reps("dot9", 2)];
  const a = analyzeHand(concealed, 1);
  assert.equal(a.sets, 1); // only the one she locked is "done"
  assert.equal(a.hasPair, true); // she holds a pair
  assert.equal(a.setSlots.filter((s) => s === "done").length, 1);
  assert.ok(a.setSlots.includes("partial")); // the in-hand triple/pair are in progress
  assert.ok(a.wantComplete.includes("crak5"));
});

test("analyzeHand: a hand of singles + many Jokers shows NO done sets (regression)", () => {
  // The bug: a fresh joker-heavy hand of all-singles reported sets already
  // "locked in". A set is only counted when truly held (triple or pair+Joker).
  const concealed = [
    tile("crak7"), tile("bam2"), tile("dot4"), tile("dot5"), tile("dot9"),
    tile("E"), tile("S"), tile("W"), tile("RD"),
    joker(), joker(), joker(), joker(), joker(),
  ];
  const a = analyzeHand(concealed, 0);
  assert.equal(a.sets, 0);
  assert.equal(a.hasPair, false);
  assert.equal(a.setSlots.filter((s) => s === "done").length, 0);
});

test("analyzeHand: an uncommitted pair+Joker is 'in progress', not done", () => {
  const a = analyzeHand([...reps("bam3", 2), ...reps("dot1", 2), joker()], 0);
  assert.equal(a.sets, 0); // nothing locked yet
  assert.equal(a.hasPair, true); // one pair is reserved as THE pair
  assert.ok(a.setSlots.includes("partial")); // the other pair + Joker shows as building
});
...
test("decomposeWin: returns 4 sets + a real pair that account for all 14 tiles", () => {
  // The screenshot-style win: lots of jokers + scattered singles.
  const hand = [
    ...reps("crak5", 4), ...reps("crak3", 2), tile("crak2"), tile("bam2"),
    joker(), joker(), joker(), joker(), joker(), joker(),
  ];
  assert.equal(hand.length, 14);
  const d = decomposeWin(hand);
  assert.ok(d, "should find a winning decomposition");
  assert.equal(d.sets.length, 4);
  assert.equal(d.pair.length, 2);
  // every set is exactly 3 tiles
  for (const s of d.sets) assert.equal(s.length, 3);
  // the pair is two identical REAL tiles (no jokers)
  assert.equal(d.pair[0].isJoker, false);
  assert.equal(d.pair[1].isJoker, false);
  assert.equal(d.pair[0].key, d.pair[1].key);
  // it accounts for all 14 tiles exactly once (by id)
  const usedIds = [...d.sets.flat(), ...d.pair].map((t) => t.id).sort();
  assert.deepEqual(usedIds, hand.map((t) => t.id).sort());
});

test("decomposeWin: returns null for a non-winning hand", () => {
  const notWin = [
  ...
  assert.equal(decomposeWin(notWin), null);
});
...
test("canFormSets: three or four of a key both work; leftover is rejected", () => {
  assert.equal(canFormSets({ bam1: 4 }, 0, 1), true); // a kong
  assert.equal(canFormSets({ bam1: 3 }, 0, 1), true); // a pung
  assert.equal(canFormSets({ bam1: 2 }, 1, 1), true); // pung with a joker
  assert.equal(canFormSets({ bam1: 5 }, 0, 1), false); // 5 can't be one set
});

test("decomposeWin: returns a set of four for a kong hand", () => {
  const hand = [
    ...reps("bam1", 4), ...reps("bam2", 3), ...reps("crak5", 3), ...reps("RD", 3), ...reps("dot9", 2),
  ];
  const d = decomposeWin(hand);
  assert.ok(d);
  assert.equal(d.sets.length, 4);
  assert.ok(d.sets.some((s) => s.length === 4)); // the kong
  const usedIds = [...d.sets.flat(), ...d.pair].map((t) => t.id).sort();
  assert.deepEqual(usedIds, hand.map((t) => t.id).sort());
});
